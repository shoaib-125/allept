...
class RegistrationController extends Controller
{
    ...
    public function create()
    {
        ...
        {
            $data['customer_group_id'] = $this->customerGroupRepository->findOneWhere(['code' => 'wholesale'])->id;
        }else {
        ...
        if ($customer) {
            ...
            /** if customer is created then create it as admin if it's a company */
            if ( $data['customer-type'] == 'company' )
            {
                $adminData = $this->prepareAdminFields($data);

                Admin::createAdmin($adminData);
            }
    ...
    protected function prepareAdminFields($data)
    {
        $data['role_name'] = 'whole seller';
        $data['status'] = 1;
